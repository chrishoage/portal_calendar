...
void setup()
{
    ...
    #ifdef SHOW_WEATHER
    prefs.begin("PortalCalender"); //namespace
    #endif // SHOW_WEATHER
    ...
    #else
    bool showWeather = prefs.getBool("showWeather", 1);

    DEBUG_PRINT("Show Weather value %u", showWeather);
    if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT1) {
        DEBUG_PRINT("Wake from button press changing showWeather from %u to %u", showWeather, !showWeather);

        showWeather = !showWeather;
        prefs.putBool("showWeather", showWeather);
